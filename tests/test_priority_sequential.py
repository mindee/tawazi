...
def test_priority() -> None:
    ...
    for _i in range(100):
        ...
        en_c = ExecNode(c.__name__, c, [UsageExecNode(en_b)], priority=2, is_sequential=False)
        en_d = ExecNode(d.__name__, d, [UsageExecNode(en_a)], priority=1, is_sequential=False)
...
def test_sequentiality() -> None:
    for _i in range(100):
        ...
        en_b = ExecNode(b.__name__, b, [UsageExecNode(en_a)], priority=2, is_sequential=False)
        en_c = ExecNode(c.__name__, c, [UsageExecNode(en_a)], priority=2, is_sequential=False)
        en_d = ExecNode(d.__name__, d, [UsageExecNode(en_b)], priority=2, is_sequential=False)
        en_e = ExecNode(e.__name__, e, [UsageExecNode(en_a)], priority=1, is_sequential=True)
