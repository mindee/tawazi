...
def test_priority() -> None:
    ...
    for _i in range(100):
        pytest.priority_sequential_comp_str = ""
        en_a = XNWrapper(ExecNode(a.__name__, a, priority=1, is_sequential=False))
        en_b = XNWrapper(ExecNode(b.__name__, b, [en_a], priority=2, is_sequential=False))
        en_c = XNWrapper(ExecNode(c.__name__, c, [en_b], priority=2, is_sequential=False))
        en_d = XNWrapper(ExecNode(d.__name__, d, [en_a], priority=1, is_sequential=False))
...
def test_sequentiality() -> None:
    for _i in range(100):
        # Sequentiality test
        pytest.priority_sequential_comp_str = ""
        en_a = XNWrapper(ExecNode(a.__name__, a, is_sequential=False))
        en_b = XNWrapper(ExecNode(b.__name__, b, [en_a], priority=2, is_sequential=False))
        en_c = XNWrapper(ExecNode(c.__name__, c, [en_a], priority=2, is_sequential=False))
        en_d = XNWrapper(ExecNode(d.__name__, d, [en_b], priority=2, is_sequential=False))
        en_e = XNWrapper(ExecNode(e.__name__, e, [en_a], priority=1, is_sequential=True))
