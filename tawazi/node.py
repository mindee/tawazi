...
class LazyExecNode(ExecNode, Generic[P, RVXN]):
    ...
    def __call__(
        ...
        return XNWrapper(self_copy)  # type: ignore
...
class XNWrapper:
    ...
    def result(self) -> Any:
        if isinstance(self.key, NoValType):
            ...
            return self.xn.result[self.key]  # type: ignore
...
def get_return_ids(returned_exec_nodes: ReturnXNsType) -> ReturnIDsType:
    ...
    elif isinstance(returned_exec_nodes, XNWrapper):
        return_ids = returned_exec_nodes.xn.id
    # 4 multiple values returned
        ...
        for ren in returned_exec_nodes:
            if isinstance(ren, XNWrapper):
                return_ids.append(ren.xn.id)  # type: ignore
            else:
